...
const Contact = ({ subscribeDisplay, setSubscribeDisplay }: contactProps) => {
  ...
  const handleSubscription = async (
    ...
    if (email !== "" || name !== "" || birthday !== "") {
      ...
      jsonp(
        `${url}&EMAIL=${email}&FNAME=${name}&BIRTHDAY[month]=${bMonth}&BIRTHDAY[day]=${bDay}`,
        { param: "c" },
        (_, { msg }) => {
          // console.log(msg);
        ...
        {!success ? (
          <div>
            <h2 className="flex items-center justify-center mb-5">
              ...
              <MdClose
                className="ml-auto text-3xl cursor-pointer black"
            ...
            >
              <div className="flex flex-col items-start space-y-3">
                <label
                  htmlFor="mce-FNAME"
                  className="flex items-center space-x-1 font-medium lowercase"
                >
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 text-center text-white uppercase bg-gray-800 rounded-md hover:bg-gray-900"
              >
